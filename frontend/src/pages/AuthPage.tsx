...
/**
 * Dark, focused auth screen at /login and /register. Supports password login,
 * account creation, and Google OAuth. After a successful auth the parent
 * (App) resumes any pending checkout, so this page just needs to authenticate.
 */
export function AuthPage({ mode, onLogin, onRegister, onNavigate, pendingLabel }: AuthPageProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // On success the parent reacts to the new user (and resumes checkout).
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0c12] text-gray-200 font-sans flex flex-col items-center justify-center px-4 relative overflow-hidden">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[420px] bg-[radial-gradient(ellipse_60%_60%_at_50%_-10%,rgba(99,91,255,0.18),transparent)]" />
      ...
      <button onClick={() => onNavigate('/')} className="relative z-10 flex items-center gap-3 mb-8">
        ...
        <span className="text-white font-extrabold tracking-tight text-lg">ARIES</span>
      </button>
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="relative rounded-3xl border border-white/10 bg-[#10131c]/90 backdrop-blur p-7 shadow-2xl">
          {pendingLabel && (
            <div className="mb-5 flex items-start gap-2.5 rounded-xl border border-indigo-400/20 bg-indigo-500/10 px-4 py-3">
              <ShoppingCart className="w-4 h-4 text-indigo-300 mt-0.5 shrink-0" />
              <p className="text-[13px] text-indigo-200">
                {isRegister ? 'Create your account' : 'Sign in'} to continue to your{' '}
                <strong className="text-white">{pendingLabel}</strong> — you'll go straight to secure checkout.
              </p>
            </div>
          )}
          ...
          <p className="text-[13px] text-gray-500 text-center mt-1.5 mb-6">
          ...
          <div className="flex items-center gap-3 my-5">
            ...
            <span className="text-[10px] font-bold tracking-[0.15em] text-gray-500 uppercase">or</span>
            <div className="h-px flex-1 bg-white/10" />
          </div>

          {error && (
            <div className="mb-4 flex items-start gap-2 text-[12px] text-red-300 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            {isRegister && (
              <DarkField label="Full name" icon={<User className="w-4 h-4" />} type="text"
                value={name} onChange={setName} placeholder="Jane Smith" required />
            )}
            <DarkField label="Email" icon={<Mail className="w-4 h-4" />} type="email"
              value={email} onChange={setEmail} placeholder="you@company.com" required />
            <DarkField label="Password" icon={<Lock className="w-4 h-4" />} type="password"
...
function DarkField({
  ...
  return (
    <label className="block">
      <span className="block text-[10px] font-bold tracking-[0.1em] text-indigo-300 uppercase mb-1.5">{label}</span>
      <div className="relative">
        ...
        />
        <span className="text-gray-600 absolute right-3 top-1/2 -translate-y-1/2">{icon}</span>
      </div>
    </label>
  );
}
