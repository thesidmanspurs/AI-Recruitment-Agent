        ...
        <div className="relative z-10 w-full max-w-[400px]">
          ...
          <div className="mb-7">
            <h1 className="text-4xl font-normal text-gray-900 mb-2" style={{ fontFamily: "'DM Serif Display', serif" }}>
              {isRegister ? 'Create your account' : isForgot ? 'Reset password' : isReset ? 'Set new password' : 'Welcome back'}
            </h1>
          ...
          {successMessage && (
            <div className="mb-4 flex items-start gap-2 text-[12px] text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2.5">
              <Check className="w-4 h-4 mt-0.5 shrink-0 text-emerald-600" /> <span>{successMessage}</span>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            {!isForgot && (
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="block text-[10px] font-bold tracking-[0.1em] text-gray-500 uppercase">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={isRegister || isReset ? 'Min. 8 characters' : 'Your password'} required
            ...
            {isReset && (
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="block text-[10px] font-bold tracking-[0.1em] text-gray-500 uppercase">Confirm Password</span>
                </div>
                <div className="relative">
                  <span className="text-gray-400 absolute left-3.5 top-1/2 -translate-y-1/2"><Lock className="w-4 h-4" /></span>
                  <input
            ...
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" />
                : <>{isRegister ? 'Create account' : isForgot ? 'Send reset link' : isReset ? 'Update password' : 'Sign in'} <ArrowRight className="w-4 h-4" /></>}
            </button>
          </form>
