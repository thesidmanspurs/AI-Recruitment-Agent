  ...
  return (
    <MarketingShell current="features" onNavigate={onNavigate}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {SECTIONS.map(s => (
          <div key={s.title} className="p-6 bg-slate-950/40 border border-slate-900 rounded-3xl space-y-4 hover:border-slate-800 transition">
            <div className="w-10 h-10 rounded-xl bg-indigo-500/10 text-indigo-400 flex items-center justify-center border border-indigo-500/20">
              <s.icon className="w-5 h-5" />
            </div>
            <h3 className="text-sm font-bold text-white">{s.title}</h3>
            <p className="text-xs text-slate-400 leading-relaxed font-medium">{s.body}</p>
          </div>
        ))}
      </div>
