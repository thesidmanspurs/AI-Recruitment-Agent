    ...
    <MarketingShell current="home" onNavigate={onNavigate} authed={authed} onOpenWorkspace={onOpenWorkspace}>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center pt-4">
        <div className="lg:col-span-7 space-y-6">
          ...
          <p className="text-sm md:text-base text-slate-600 dark:text-slate-400 leading-relaxed max-w-xl font-medium">
          ...
          <div className="grid grid-cols-3 gap-6 pt-4 border-t border-slate-200 dark:border-slate-900/80">
            <Stat value="2,000" label="Credits / seat / mo" cls="text-gray-900 dark:text-white" />
          ...
          <div className="flex flex-wrap items-center gap-4 pt-3 text-slate-400 text-xs">
            <span className="flex items-center gap-1 bg-gray-100 dark:bg-slate-900/60 px-2.5 py-1 rounded-lg border border-gray-200 dark:border-slate-800/80 font-mono text-[10px] text-gray-600 dark:text-slate-400">
        ...
        <div className="lg:col-span-5 flex justify-center">
          {authed ? (
            <div className="w-full max-w-md bg-white dark:bg-slate-950/80 border border-gray-200 dark:border-slate-800/80 rounded-3xl p-8 shadow-2xl relative ring-1 ring-black/5 dark:ring-white/5 text-center">
              ...
              <h3 className="text-lg font-black text-gray-900 dark:text-white">You're signed in</h3>
              <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-1.5 mb-6 leading-relaxed">
          ...
          <div className="w-full max-w-md bg-white dark:bg-slate-950/80 border border-gray-200 dark:border-slate-800/80 rounded-3xl p-8 shadow-2xl relative ring-1 ring-black/5 dark:ring-white/5">
            <div className="absolute top-0 right-6 -translate-y-1/2 bg-emerald-500 text-slate-950 font-mono text-[9px] font-bold px-3 py-1 rounded-full uppercase tracking-widest shadow-md">
              Gate Lock Active
            </div>
            <div className="text-center mb-6">
              <h3 className="text-lg font-black text-gray-900 dark:text-white">Enter Recruiter Console</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <Field label="Registered Business Email" icon={<Mail className="w-4 h-4" />}>
                <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="name@agency-brand.com"
                  className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 text-gray-900 dark:text-slate-100 placeholder:text-gray-400 dark:placeholder:text-slate-600 rounded-xl py-2.5 pl-3.5 pr-10 text-xs focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50" />
              </Field>
            ...
            <div className="mt-5 p-3.5 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-800/80 rounded-2xl flex justify-between items-center text-[10.5px]">
              <div>
                <span className="text-indigo-500 dark:text-indigo-300 font-bold block mb-0.5">Need an account?</span>
                <span className="text-slate-500 dark:text-slate-400 leading-snug">Register a new recruiter workspace.</span>
              </div>
      ...
      <section className="mt-24 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {HIGHLIGHTS.map(h => (
            <div key={h.title} className="p-6 bg-gray-100 dark:bg-slate-950/40 border border-gray-200 dark:border-slate-900 rounded-3xl space-y-3 hover:border-gray-300 dark:hover:border-slate-800 transition">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${h.color}`}><h.icon className="w-5 h-5" /></div>
              <h3 className="text-base font-bold text-gray-900 dark:text-white">{h.title}</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-medium">{h.body}</p>
            </div>
          ))}
        </div>
...
function Stat({ value, label, cls }: { value: string; label: string; cls: string }) {
  return (
    <div>
      ...
      <span className="text-[10.5px] text-slate-500 dark:text-slate-500 font-mono uppercase tracking-wider block mt-1">{label}</span>
    </div>
  );
}
