...
export function LocationFilter({ applied, onApply }: LocationFilterProps) {
  ...
  // Suggestions filtered by query, with anything already in draft pinned
  // to the top so the user can see what's selected at a glance.
  const lowerQ = query.trim().toLowerCase();
  const matchedSuggestions = SUGGESTIONS.filter(s =>
    lowerQ ? s.toLowerCase().includes(lowerQ) : true
  );
  const visibleList = Array.from(new Set([...draft, ...matchedSuggestions])).slice(0, 12);
      ...
      {open && panelPos && createPortal(
        ...
        >
          ...
          <div className="p-3 border-b border-gray-100">
            ...
            {query.trim() && !SUGGESTIONS.some(s => s.toLowerCase() === lowerQ) && (
              <button
                type="button"
                onClick={addCustom}
