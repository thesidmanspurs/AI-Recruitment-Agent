            ...
            {NAV.map(n => (
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition leading-none ${
                  current === n.tab ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-white hover:bg-white/5'
                }`}
              >
                {n.label}
              </button>
      ...
      <footer className="border-t border-slate-900 bg-slate-950/80 px-6 py-10 text-slate-500 text-xs relative z-10 select-none">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-wrap gap-6 text-[11px]">
            <button onClick={() => onNavigate('/engine-features')} className="hover:text-slate-300">Features</button>
            <button onClick={() => onNavigate('/pricing')} className="hover:text-slate-300">Pricing</button>
            <button onClick={() => onNavigate('/faq')} className="hover:text-slate-300">FAQ</button>
